...
let [open, setOpen] = useState(false);
  ...
  return (
    ...
    <div className=' navbar md:flex items-center py-4 md:px-2 px-7 gap-7 ml-12  '>
    <div className='navbar font-bold   cursor-pointer flex items-center font-[Poppins] 
    text-gray-800'>
      <div className=' mr-1 p-0 m-0 '>
      <img src={LogoImage} alt="Maro_logo" className=' paga_logo '/>
      </div>
     
    </div>
    
    <div onClick={()=>setOpen(!open)} className='text-3xl absolute right-8 top-6 cursor-pointer md:hidden text-gray-900'>
    <ion-icon name={open ? 'close':'menu'} className="hamburger"></ion-icon>
    </div>
